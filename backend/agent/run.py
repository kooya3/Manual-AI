...
from agent.tools.manuals_tool import ManualsTool
...
async def run_agent(
    ...
    thread_manager.add_tool(ManualsTool, project_id=project_id, thread_manager=thread_manager)
...
async def test_agent():
    ...
    try:
        ...
        project_result = await client.table('projects').select('*').eq('name', 'test11').execute()
